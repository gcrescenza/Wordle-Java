...
public class GameManager {
    ...
    public void computeInput(String inString) throws NotRecognized {
        if (inString.charAt(0) == '/')
            ...
            // take it as a guess
            if (playStatus && actualGame != null) // Aggiunto controllo null
    ...
    private void computeCommand(String inString) throws NotRecognized {
        ...
        if (splitCommand.length == 1) {
            if (splitCommand[0].equals(Command.PLAY.toString())) {
                ...
                // FIX: Controllo se la partita è stata creata prima di giocare
                if (actualGame != null) {
                    playStatus = true;
                    play();
                } else {
                    System.out.println("⚠️ Attenzione: Devi prima impostare una parola segreta con il comando /nuova <parola>");
                }
    ...
    private boolean checkSecretWord(String inString) {
        if (inString.length() > 5) {
            ...
            System.out.println("Parola segreta impostata correttamente! Digita /gioca per iniziare.");
            return true;
        }
    }
     ...
     */
    public void play() {
        // Ulteriore controllo di sicurezza per evitare NullPointerException
        ...
        do {
            ...
            // Aggiunto controllo null anche nel while per sicurezza post-abbandono
        } while (playStatus && actualGame != null && !actualGame.hasWon() && !actualGame.hasLost());
        
        if (playStatus && actualGame != null) {
